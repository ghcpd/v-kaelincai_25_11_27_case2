...
async function runValidation() {
  try {
    ...
    log(`Node.js: ${require('child_process').execSync('node --version', { encoding: 'utf8' }).trim()}`);
    log(`npm: ${require('child_process').execSync('npm -v', { encoding: 'utf8' }).trim()}`);
    ...
    try {
      ...
      if (fs.existsSync('frontend/dist')) {
        const files = require('fs').readdirSync('frontend/dist', { recursive: true });
    ...
    try {
      ...
      log('Test execution completed with status code: ' + e.status);
